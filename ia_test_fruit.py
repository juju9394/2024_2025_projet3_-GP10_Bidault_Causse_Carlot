...
batch_size = 8  # Réduction de la taille du batch pour un traitement plus rapide
image_size = 32  # Réduction de la taille des images pour un traitement plus rapide
max_epochs = 5  # Nombre d'époques réduit
learning_rate = 0.001
model_path = "mobilenet_v2_fruits_best.pth"  # Utilisation de MobileNetV2
...
def train_model():
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0)  # Réduction des workers
    ...
    model = models.mobilenet_v2(pretrained=True)  # Utilisation de MobileNetV2
    model.classifier[1] = nn.Linear(model.last_channel, num_classes)  # Adaptation de la dernière couche
